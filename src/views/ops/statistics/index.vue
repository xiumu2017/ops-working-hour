    ...
    <div class="filter-container">
      ...
      <el-dialog :visible.sync="statisticsDialogVisiable" width="40%">
        <el-select v-model="workNo" filterable placeholder="请选择" @change="statisticsByUser">
          <el-option v-for="item in users" :key="item.workNo" :label="item.realName" :value="item.workNo">
          </el-option>
        </el-select>
        <el-select v-model="psaId" filterable placeholder="请选择" @change="statisticsByPsa">
          <el-option v-for="item in psaList" :key="item.psaId" :label="item.psaName" :value="item.psaId">
          </el-option>
        </el-select>
        ...
        <el-table v-loading="statisticsListLoading" :data="statisticsList" element-loading-text="Loading" border fit
          ...
          <el-table-column align="center" prop="created_at" label="工时占比" width="200">
            <template slot-scope="scope">
...
<script>
import { getList, getUsers, getPsaList, getUserStatistics, getPsaStatistics } from '@/api/ops/statistics'
